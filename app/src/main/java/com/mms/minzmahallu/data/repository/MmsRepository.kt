...
class MmsRepository(val db: DatabaseManager) {
    ...
    fun subscriptionPay(id: Long, amountPaid: Double, method: String, paymentDate: String, remarks: String = ""): Map<String, Any?> {
        ...
        val receipt = nextReceiptNumber(paymentDate, "subscription")
    ...
    fun markOverdue(): Int {
        // Port of desktop subscriptions.markOverdue (8b7efdc): accounts are
        // rolled to the CURRENT month by ensureCurrentMonth, so the old
        // "period_end < today" rule never fired — the button did nothing.
        // A family is overdue when its account carries past-month debt
        // (arrears > 0); recording a payment re-computes status and clears
        // the flag once the backlog is gone.
        val changed = db.run("UPDATE subscriptions SET status='Overdue', updated_at=datetime('now') WHERE status IN ('Pending','Partial') AND COALESCE(arrears,0) > 0.004")
        auth.audit("MARK_OVERDUE", "subscriptions", null, "Marked $changed account(s) overdue")
        return changed.toInt()
    }
    ...
    fun donationCreate(data: Map<String, Any?>): Map<String, Any?> {
        ...
        val receipt = data["receiptNumber"]?.toString()?.takeIf { it.isNotBlank() } ?: nextReceiptNumber(date, "donation")
    ...
    // ------------------------------------------------- document numbering
    // Port of desktop doc-number.service.ts: donations and subscriptions are
    // SEPARATE money books (codes DN vs SB), each scanning only its own
    // tables, so a donation receipt (MMJM/DN/26/09/001) can never collide
    // with a subscription receipt (MMJM/SB/26/09/001). The mahallu's own
    // letters lead every number; blank or legacy "RCP" prefix falls back to
    // initials derived from the mahallu name ("Minz Mahallu" → MIN).
    private fun sanitizePrefix(raw: String?): String {
        val letters = (raw ?: "").uppercase().filter { it in 'A'..'Z' }
        return letters.take(5)
    }

    private fun mahalluInitials(name: String): String {
        val upper = name.uppercase()
        val words = upper.replace(Regex("[^A-Z\\s]"), " ").trim().split(Regex("\\s+")).filter { it.isNotBlank() }
        if (words.isEmpty()) return ""
        val initials = words.take(4).map { it[0] }.joinToString("")
        return if (initials.length >= 3) initials else upper.filter { it in 'A'..'Z' }.take(3)
    }

    private fun mahalluPrefix(): String {
        val s = settingsLoad()
        val fallback = mahalluInitials(Format.str(s, "mahallu_name")).ifBlank { "RC" }
        val letters = sanitizePrefix(Format.str(s, "receipt_prefix"))
        return if (letters.isNotEmpty() && letters != "RCP") letters else fallback
    }

    private fun yearMonthOf(date: String): Pair<String, String> {
        val m = Regex("^(\\d{4})-(\\d{2})").find(date)
        if (m != null) return Pair(m.groupValues[1].substring(2), m.groupValues[2])
        val t = Format.today()
        return if (t.length >= 7) Pair(t.substring(2, 4), t.substring(5, 7)) else Pair("00", "01")
    }

    private fun nextReceiptNumber(date: String, kind: String): String {
        val (y, m) = yearMonthOf(date)
        val code = if (kind == "subscription") "SB" else "DN"
        val head = "${mahalluPrefix()}/$code/$y/$m/"
        val used = if (kind == "subscription")
            runCatching {
                db.all("SELECT receipt_number AS n FROM subscription_payments WHERE receipt_number IS NOT NULL AND receipt_number != ''") +
                    db.all("SELECT receipt_number AS n FROM subscriptions WHERE receipt_number IS NOT NULL AND receipt_number != ''")
            }.getOrDefault(emptyList())
        else
            db.all("SELECT receipt_number AS n FROM donations WHERE receipt_number IS NOT NULL AND receipt_number != ''")
        var max = 0
        for (r in used) {
            val n = Format.str(r, "n")
            if (n.startsWith(head)) n.substring(head.length).toIntOrNull()?.let { if (it > max) max = it }
        }
        return "$head${(max + 1).toString().padStart(3, '0')}"
    }

    /** Wipes every business table (children before parents) and keeps
     *  users/settings/plans/categories — the production "start clean" port
     *  of desktop clear-data.service.ts. Returns rows erased. */
    fun clearAllData(): Int {
        val tables = listOf(
            "audit_chain", "audit_log", "notifications", "documents", "certificates",
            "token_events", "token_assignments", "welfare_requests", "marriages", "deaths",
            "subscription_payments", "subscriptions", "donations", "transactions",
            "staff_payments", "staff", "committee_members", "family_moves",
            "record_history", "assets", "members", "families",
        )
        var cleared = 0
        runCatching { db.exec("PRAGMA foreign_keys = OFF") }
        for (t in tables) {
            try { cleared += db.run("DELETE FROM $t").toInt() } catch (_: Exception) { /* table missing on older schema */ }
        }
        try {
            db.run("DELETE FROM sqlite_sequence WHERE name IN (${tables.joinToString(",") { "?" }})", tables.toTypedArray())
        } catch (_: Exception) { }
        runCatching { db.exec("VACUUM") }
        runCatching { db.exec("PRAGMA foreign_keys = ON") }
        auth.audit("CLEAR_ALL_DATA", "system", null, "All business data cleared (genesis)")
        return cleared
    }
    ...
    fun appInfo() = mapOf("name" to "Minz Mahallu Management System", "version" to BuildConfig.VERSION_NAME, "platform" to "Android")
